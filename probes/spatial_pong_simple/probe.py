...
def score_pong_simple(response: str, expected: str) -> float:
    ...
    # Find the LAST occurrence of any action word
    # Handles: "I need to decide if I go up or down" -> "down" (last match)
    last_word = None
    last_pos = -1
...
class SpatialPongSimpleProbe(BaseProbe):
    ...
    def run(self, model) -> dict:
        ...
        easy_scores, easy_results = self._run_items(
            ...
            prompt_fn=_prompt,
            score_fn=lambda resp, item: score_pong_simple(resp, item["answer"]),
